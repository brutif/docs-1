...
.. If installing the optional Keycloak component for test and development use:
... Your cluster must support persistent volumes.  Create a persistent volume of type ReadWriteOnce with capacity of at least 1GB. 
... Specify `yes` on the ENABLE_KEYCLOAK environment variable.  If not, specify `no`.
... `ENABLE_KEYCLOAK=yes|no ./install.sh`
